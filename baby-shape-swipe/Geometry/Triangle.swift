...
class Triangle: Shape {
    ...
    static func makeScreenSpaceDefault() -> Triangle {
        ...
        switch random {
        case 1:
            point1 = mapToScreen (Point(x: 5, y: 5))
            point2 = mapToScreen (Point(x: 1, y: 4))
            point3 = mapToScreen (Point(x: 2, y: 9))
        case 2:
            point1 = mapToScreen (Point(x: 1, y: 2))
            point2 = mapToScreen (Point(x: 5, y: 6))
            point3 = mapToScreen (Point(x: 1, y: 4))
        default:
            point1 = mapToScreen (Point(x: 2, y: 2))
            point2 = mapToScreen (Point(x: 4, y: 7))
            point3 = mapToScreen (Point(x: 1, y: 5))
        }
    ...
    var description: String { return "triangle: p1: \(mapToModel(p1).description), p2: \(mapToModel(p2).description), p3: \(mapToModel(p3).description)" }
    var p1: Point
    var p2: Point
    var p3: Point
    var color: UIColor
